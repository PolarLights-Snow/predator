...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    mcu_dp_bool_update(DPID_QUICK_FEED,当前自动喂食开关); //BOOL型数据上报;
  ...
  //  mcu_dp_value_update(DPID_VOICE_TIMES,当前语音播放次数); //VALUE型数据上报;
  //  mcu_dp_bool_update(DPID_SWITCH,当前开关); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_QUICK_FEED,Flag_ZD_KG); //当前自动喂食开关  BOOL型数据上报;
    mcu_dp_enum_update(DPID_FEED_STATE,0); //当前喂食状态  枚举型数据上报;		0:准备     1：喂食			2：完成
    mcu_dp_value_update(DPID_SURPLUS_GRAIN,Flag_YL_LT); //当前粮桶余粮  VALUE型数据上报;
    mcu_dp_value_update(DPID_WEIGHT,Flag_YL_SW); //当前余粮重量   VALUE型数据上报;
}
